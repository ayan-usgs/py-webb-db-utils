...
PIEZO_SITES = (
               ...
               "WHERE ( ( SMP.RECORD_NUMBER = C.RECORD_NUMBER(+) ) AND ( SMP.RECORD_NUMBER = FD.RECORD_NUMBER ) AND ( STE.STATION_NO = SMP.STATION_NO ) ) AND ( STE.SITE_GROUP_CD IN (%s) ) AND ( SMP.SAMPLE_DATE >= :StartDate ) AND ( SMP.SAMPLE_DATE <= :EndDate ) "
...
WELL_CK_VALUES = (
                  ...
                  "WHERE ( ( VWS.STATION_NO = WHM.STATION_NO ) AND ( VWS.STATION_NO = WHMP.STATION_NO ) ) AND ( VWS.SHORT_NAME IN (%s) ) AND ( WHM.MEAS_DATE <= :EndDT ) AND ( WHM.MEAS_DATE >= :StartDT )"
...
DATA_WITH_UV = (
                ...
                "WHERE ( ( SMP.RECORD_NUMBER = ANN.RECORD_NUMBER(+) ) AND ( STE.STATION_NO = SMP.STATION_NO ) ) AND ( STE.SITE_GROUP_CD IN (%s) ) AND ( SMP.SAMPLE_DATE >= :StartDate ) AND ( SMP.SAMPLE_DATE <= :EndDate ) "
...
WITH_DATA = (
             ...
             "WHERE ( ( SMP.RECORD_NUMBER = CAR.RECORD_NUMBER(+) ) AND ( SMP.RECORD_NUMBER = FLD.RECORD_NUMBER ) AND ( STE.STATION_NO = SMP.STATION_NO ) ) AND ( STE.SITE_GROUP_CD IN (%s) ) AND ( SMP.SAMPLE_DATE >= :StartDate ) AND ( SMP.SAMPLE_DATE <= :EndDate ) "
...
WELL_UVS = (
            ...
            "WHERE ( ( VWS.STATION_NO = VR.STATION_NO ) ) AND ( VWS.SHORT_NAME IN (%s) ) AND ( VR.RESULT_DATETIME <= :EndUVDate ) AND ( VR.RESULT_DATETIME >= :StartUVDate ) AND ( VR.PARAMETER_CODE IN (( '72019' )) )"
            )
